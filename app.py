...
def wikipedia_search(query):
    """Wikipedia API — free, no key. Optimised for Vercel free tier (hard 4s cap)."""
    try:
        resp = requests.get(
        ...
        if not items:
            print(f"[WIKI] No results for: {query}")
            return None
        results = []
        for item in items:
        ...
        print(f"[WIKI] {len(results)} results for: {query}")
        return results
    except requests.exceptions.Timeout:
        print(f"[WIKI] Timeout on: {query}")
        return None
    except Exception as e:
...
def _do_web_search(query, user_msg=None):
    """
    Wikipedia-backed web search with Groq summarisation.
    Total budget kept under 8s so Vercel free (10s limit) never kills it.
      Wiki call : max 4s
      Groq call : max 4s
    """
    ...
    sources_html = (
        ...
        '<div class="wsearch-header">🔍 Web Results</div>'
    ...
    if groq_key and user_msg:
        try:
            ...
            print(f"[SEARCH] Groq OK — {len(summary)} chars")
        except requests.exceptions.Timeout:
            print("[SEARCH] Groq timeout — cards only")
        except Exception as e:
            print(f"[SEARCH] Groq error: {e}")
...
# Backward-compat alias — all existing call sites still work
_do_google_search = _do_web_search
