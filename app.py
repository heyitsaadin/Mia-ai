...
def _build_clean_history(history):
    ...
    # ── Pass 2: build clean entries from all history (skip noise) ──
    def _to_entry(msg):
        txt = msg["text"]
        if txt.startswith("[IMAGE GENERATED:"):
        ...
        if txt.startswith("[IMAGE ANALYSIS RESULT]"):
            return None  # injected via system prompt, skip
    ...
    # Strategy:
    #   - Try to send ALL entries if their total raw content fits under SOFT_LIMIT chars.
    #   - If total is over SOFT_LIMIT, fall back to last WINDOW_MIN entries.
    #   - Each individual message is hard-capped at MSG_CAP chars regardless.
    #   - The very first user message is always included as a context anchor.
    SOFT_LIMIT = 1200   # chars — under this, send everything (small chat)
    WINDOW_MIN = 4      # entries — fallback window for big chats
    MSG_CAP    = 250    # chars — hard cap per individual message

    # Apply per-message cap
    capped = []
    for e in all_entries:
        capped.append({
            "role": e["role"],
            "content": e["content"][:MSG_CAP] if len(e["content"]) > MSG_CAP else e["content"]
        })

    total_chars = sum(len(e["content"]) for e in capped)

    if total_chars <= SOFT_LIMIT:
        # Small chat — send everything, full context preserved
        clean_msgs = capped
    else:
        # Big chat — send only last WINDOW_MIN entries
        clean_msgs = capped[-WINDOW_MIN:]
        # Always anchor with the first real user message so Jarvis
        # remembers how the conversation started
...
→ image_prompt should be empty string.
→ MULTI-ACTION: If the user also wants text (e.g. a recipe AND a video), set action to "youtube_search", put the search query in reply, AND also set youtube_plus_text to the text response you want shown above the video.
...
YOU MUST ALWAYS RESPOND IN THIS EXACT JSON FORMAT — no markdown, no explanation outside the JSON:
{
  ...
  "reply": "your conversational reply, math expression, or YouTube search query depending on action",
  ...
  "youtube_plus_text": "if action is youtube_search AND user also wants a text response (e.g. a recipe), put that text here. Empty string otherwise."
}"""
    if last_img_prompt:
...
def _build_reply(user_msg):
    ...
    if action == "youtube_search":
        ...
        if plus_text:
            # Combine: text reply first, then video below, separated by a marker
