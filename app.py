...
def get_db():
    ...
    try:
        ...
        # Pool exhausted or broken — reinitialise and retry once
        db_pool = None
        init_db_pool()
        return db_pool.getconn()
...
def is_banned(value, ban_type):
    ...
    try:
        ...
        return False  # fail-open: if DB is down, don't block every request
    finally:
        if conn:
            return_db(conn)
...
def check_ban():
    ...
    try:
        ...
        pass  # DB error — let the request through rather than blank-paging everyone
...
def _build_clean_history(history):
    ...
    for msg in history[-20:]:
        ...
        if total_chars + entry_len > MAX_HISTORY_CHARS:
            continue  # skip this message but keep scanning for smaller ones
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 1:
                ...
                try:
                    if 'raw' in locals() and isinstance(raw, dict) and 'choices' in raw:
            ...
            # attempt 0 failed — clear raw so attempt 1 doesn't see stale data
            raw = {}
    return {
        "action":        "text",
...
def _detect_youtube_request(user_msg):
    ...
    if not any(t in msg for t in _YT_TRIGGERS):
        return ""
    # Strip filler words to build a clean search query — operate on lowercased copy
    # but preserve original casing in the final query
    query = user_msg.strip()
    query_lower = query.lower()
    for filler in ["can you", "could you", "please", "show me", "find me",
                   "give me", "i want", "i need", "get me", "a ", "an ",
                   "some ", "the "]:
        # Replace in lowercased version and mirror removal back to original
        while filler in query_lower:
            idx = query_lower.find(filler)
            query = query[:idx] + " " + query[idx + len(filler):]
            query_lower = query_lower[:idx] + " " + query_lower[idx + len(filler):]
