...
def _auto_analyse_generated_image(img_prompt, session_messages):
    """Fetch the just-generated Pollinations image, run it through the vision model,
    and silently append a hidden [IMAGE ANALYSIS RESULT] entry to session history.
    Called in a background thread right after every image generation so the text model
    always has accurate visual context ready — no re-fetching needed on follow-ups."""
    ...
    try:
        ...
        # Pollinations may take a moment to render — give it up to 40s
        ...
        if description:
            # Inject silently — hidden from the user, visible to the text model via ask_jarvis
            session_messages.append({
                "sender": "Jarvis",
                "text":   f"[IMAGE ANALYSIS RESULT]\n{description}"
            })
    except Exception:
        pass  # Silent failure — the chat still works, just without pre-analysis
...
4. You have no knowledge of any verification codes. Never guess or invent them."""
    ...
    for msg in history[-20:]:
        ...
        if 'jarvis-img-wrap' in txt:
            ...
            # Strip the marker and pass the analysis as trusted assistant context
            analysis_text = txt[len("[IMAGE ANALYSIS RESULT]"):].strip()
            last_img_analysis = analysis_text
            clean_msgs.append({"role": "assistant", "content": f"[I analysed an uploaded image. Here is exactly what it contained:\n{analysis_text}]"})
        else:
    ...
    if last_img_analysis:
        final_system += (
            "\n\nIMAGE CONTEXT: Earlier in this conversation you analysed an uploaded image. "
            "The analysis result is in your message history marked with [I analysed an uploaded image...]. "
            "Treat that analysis as ground truth — NEVER say you don't know what was in the image, "
            "and NEVER contradict or forget details from it when answering follow-up questions."
        )
    if last_img_prompt:
        final_system += (
...
def _build_reply(user_msg):
    ...
    if session["awaiting_owner_code"]:
        ...
        # Silently analyse the generated image in the background so the text model
        # has accurate visual context for any follow-up questions
        threading.Thread(
            target=_auto_analyse_generated_image,
            args=(prompt, session["messages"]),
            daemon=True
        ).start()
        ...
        if _is_explain and _last_gen_prompt:
            ...
            for _m in reversed(session.get("messages", [])):
                if _m.get("text", "").startswith("[IMAGE ANALYSIS RESULT]"):
                    _pre_analysis = _m["text"][len("[IMAGE ANALYSIS RESULT]"):].strip()
                    break
            if _pre_analysis:
                # Re-use the pre-computed analysis — no extra API call needed
                reply = f"[IMAGE ANALYSIS RESULT]\n{_pre_analysis}"
                return reply
            # Fallback: background analysis hasn't finished yet — do it on-demand now
        ...
        if "##OWNER_CLAIM##" in raw_reply:
            ...
            threading.Thread(
                target=_auto_analyse_generated_image,
                args=(img_prompt, session["messages"]),
                daemon=True
            ).start()
        else:
            reply = raw_reply
...
def generate_image_route():
    ...
    # Silently analyse in background for accurate follow-up context
    threading.Thread(
        target=_auto_analyse_generated_image,
        args=(prompt, session["messages"]),
        daemon=True
